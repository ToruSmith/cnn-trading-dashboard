"""
訓練引擎：
- get_dataset() 移到 asyncio.to_thread() 避免阻塞 event loop
- 每個 epoch 結束後透過 WebSocket 推播進度
- 訓練完成後生成混淆矩陣、分類報告、.md 報告
"""
import asyncio
import json
import time
...
class Trainer:
    def __init__(self, config: TrainingConfig, push_fn: Optional[Callable] = None):
        ...
        self.device = torch.device("cpu")   # Render 免費方案無 GPU
    ...
    async def run(self):
        ...
        # ── 關鍵修正：用 to_thread 避免阻塞 event loop ──────────
        try:
            ...
            await self._push("error", None, f"資料載入失敗：{e}")
            self.is_running = False
            return

        await self._push("status", "building_model",
                         f"資料載入完成，共 {len(X)} 筆。建構模型中...")
        ...
        # 優化器 & 損失
        opt_map = {"Adam": optim.Adam, "SGD": optim.SGD, "AdamW": optim.AdamW}
        opt_cls = opt_map.get(cfg.optimizer, optim.Adam)
        opt_kw  = {"lr": cfg.lr}
        if cfg.optimizer == "SGD":
            opt_kw["momentum"] = 0.9
        optimizer = opt_cls(self.model.parameters(), **opt_kw)
        ...
        for epoch in range(1, cfg.epochs + 1):
            ...
            # ── Train（移到 thread，避免阻塞 event loop）──────────
            def train_one_epoch():
                self.model.train()
                total_loss = 0.0
            ...
            def val_one_epoch():
                ...
                with torch.no_grad():
                    for xb, yb in val_loader:
                        ...
                        total   += len(yb)
                return total_loss / n_val, correct / total

            train_loss          = await asyncio.to_thread(train_one_epoch)
            val_loss, accuracy  = await asyncio.to_thread(val_one_epoch)
        ...
        metrics = await asyncio.to_thread(self._evaluate_sync, val_loader, n_val)
    ...
    def _evaluate_sync(self, val_loader, n_val):
        """同步版評估，供 to_thread 呼叫"""
        self.model.eval()
    ...
    def _generate_md_report(self, metrics: Dict) -> str:
        ...
        lines = [
            f"# ECG CNN 分析報告",
            f"**生成時間**: {now}  ",
            f"**資料來源**: {cfg.data_source}  ",
            f"**模型架構**: {cfg.conv_layers} 卷積層，Kernel={cfg.kernel_size}，Dropout={cfg.dropout}  ",
            f"**訓練設定**: LR={cfg.lr}，Batch={cfg.batch_size}，Epochs={cfg.epochs}，Loss={cfg.loss_fn}",
            "",
            "---",
            "",
            "## 訓練摘要",
            ...
            "| 指標 | 數值 |",
            "|------|------|",
            f"| 最終驗證準確率 | **{metrics['accuracy']:.2%}** |",
            f"| 訓練 Epochs | {cfg.epochs} |",
            f"| 模型參數量 | {sum(p.numel() for p in self.model.parameters()):,} |",
            "",
            "## 分類效能",
            "",
        ...
        for cls_name in CLASS_NAMES[:n_cls]:
            r = rpt.get(cls_name, {})
            lines.append(
                f"| {cls_name} | {r.get('precision',0):.3f} | "
                f"{r.get('recall',0):.3f} | {r.get('f1-score',0):.3f} | "
                f"{int(r.get('support',0))} |"
            )
        macro = rpt.get("macro avg", {})
        lines.append(
            f"| **Macro Avg** | {macro.get('precision',0):.3f} | "
            f"{macro.get('recall',0):.3f} | {macro.get('f1-score',0):.3f} | — |"
        )
        ...
        for i in range(start, len(h["train_loss"])):
            lines.append(
                f"| {i+1} | {h['train_loss'][i]} | "
                f"{h['val_loss'][i]} | {h['accuracy'][i]:.2%} |"
            )
        ...
        lines += [
            ...
            "## 結論與建議",
            "",
            "1. 若 Ventricular (V) Recall < 0.85，改用 FocalLoss 或增加卷積層數。",
            "2. 若 val_loss > train_loss × 1.5，提高 Dropout 或減少層數。",
            "3. 正式向董事報告前，建議切換 mitbih 資料來源進行最終驗證。",
            "",
    ...
    async def _push(self, event: str, data: Any = None, message: str = ""):
        payload = json.dumps({"event": event, "data": data, "message": message})
